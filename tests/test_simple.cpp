 ...
 * @date 2025
 *
 * Versao simplificada dos testes para executar sem dependencias externas.
...
void test_produto_basico() {
    std::cout << "\n=== Teste: Produto Basico (EU004) ===" << std::endl;

    Produto p("Tomate", 3.50, "Hortifruti", "Feira Teste");

    TEST_ASSERT(p.GetNome() == "Tomate", "Nome do produto correto");
    TEST_ASSERT(std::abs(p.GetPreco() - 3.50) < 0.01, "Preco do produto correto");
    ...
    TEST_ASSERT(!p.GetNome().empty(), "Nome nao vazio");
}

void test_location_basico() {
    std::cout << "\n=== Teste: Location Basico (EU003) ===" << std::endl;

    Location loc1(-15.817, -48.112);  // Ceilandia
    ...
    TEST_ASSERT(distancia > 15.0 && distancia < 25.0, "Distancia Ceilandia-Asa Sul (~19km)");
}

void test_feira_basico() {
    std::cout << "\n=== Teste: Feira Basica (EU004) ===" << std::endl;
    ...
    TEST_ASSERT(feira.GetEndereco() == "Endereco Teste", "Endereco da feira correto");
...
void test_usuario_basico() {
    std::cout << "\n=== Teste: Usuario Basico (EU005) ===" << std::endl;

    Location loc(-15.817, -48.112);
    Usuario usuario("Joao", loc);

    TEST_ASSERT(usuario.GetNome() == "Joao", "Nome do usuario correto");
    TEST_ASSERT(!usuario.GetNome().empty(), "Nome nao vazio");
}
...
void test_remover_produto_feira() {
    ...
    TEST_ASSERT(feira.GetProdutos().size() == 0, "Feira sem produtos apos remocao");

    bool nao_removido = feira.RemoverProduto("Batata");
    TEST_ASSERT(nao_removido == false, "Remocao de produto inexistente retorna false");
}

void test_distancia_haversine() {
    std::cout << "\n=== Teste: Distancia Haversine (EU003) ===" << std::endl;

    // Ceilandia e Asa Sul - distancia real ~19km
    Location ceilandia(-15.817, -48.112);
    Location asa_sul(-15.794, -47.883);

    double dist1 = ceilandia.DistanciaPara(asa_sul);
    TEST_ASSERT(dist1 > 15.0 && dist1 < 25.0, "Distancia entre 15-25km");

    // Mesma localizacao - distancia zero
    Location mesmo1(-15.817, -48.112);
    Location mesmo2(-15.817, -48.112);

    double dist2 = mesmo1.DistanciaPara(mesmo2);
    TEST_ASSERT(dist2 < 0.1, "Distancia zero para mesma localizacao");
}

void test_feira_distancia() {
    std::cout << "\n=== Teste: Distancia de Feira (EU003) ===" << std::endl;

    Location loc_feira(-15.817, -48.112);
    Feira feira("Feira Ceilandia", "Endereco", loc_feira);

    Location loc_usuario(-15.794, -47.883);

    double distancia = feira.DistanciaPara(loc_usuario);
    TEST_ASSERT(distancia > 15.0 && distancia < 25.0, "Distancia feira-usuario correta");
}
...
void test_produto_getters_setters() {
    ...
    TEST_ASSERT(p.GetNome() == "Maca", "SetNome/GetNome funcionando");
...
int main() {
    ...
    // Relatorio final
    std::cout << "\n============================================" << std::endl;
    std::cout << "   RELATORIO DE TESTES" << std::endl;
    std::cout << "============================================" << std::endl;
    ...
    if (tests_failed == 0) {
        ...
        std::cout << "  - EU003 (Calcular Distancia): TESTADO" << std::endl;
        std::cout << "  - EU004 (Adicionar Produtos): TESTADO" << std::endl;
        std::cout << "  - EU005 (Cadastrar Usuario): TESTADO" << std::endl;
        return 0;
    } else {
